...
\end{verbatim}
    \item Avviare il simulatore con \texttt{Wokwi: Start Simulator} dal \textbf{Command Palette}.
    \item Wokwi caricherà automaticamente lo schema in \texttt{diagram.json} insieme al firmware appena compilato.
\end{enumerate}
...
#define PRODUCTION  true
    \end{lstlisting}
    \item Collegare la scheda Elegoo UNO R3 via USB al PC.
    \item Dal \textbf{Command Palette}, selezionare \texttt{PlatformIO: Upload}.
          PlatformIO compilerà e caricherà il firmware direttamente sulla board.
    \item Verificare che il programma parta correttamente: il display a matrice e il LCD mostreranno lo stato del gioco.
\end{enumerate}

\subsection{Note su Wokwi \& PlatformIO}
Per lavorare in entrambe le modalità è sufficiente mantenere la cartella aperta come progetto PlatformIO: VS Code riconosce automaticamente il file \texttt{platformio.ini} e propone le azioni di build/upload nella barra inferiore.
La netlist del circuito per Wokwi è definita in \texttt{diagram.json} e non richiede ulteriori modifiche.
Per modificare i codici infrarossi utilizzati dal telecomando in ambiente simulato o reale, fare riferimento a \texttt{lib/ir\_utils/ir\_utils.h}.
